...
namespace ctrls
{
	...
	//group
	group::group(nana::window wd, properties_collection* properties, const std::string& name)
		: nana::group(wd, "")
	{
	...
	void group::update(properties_collection* properties)
	{
	...
	void group::generatecode(properties_collection* properties, code_data_struct* cd, code_info_struct* ci)
	{
		// headers
	...
	void group::_initproperties(properties_collection* properties, const std::string& name)
	{
		// properties - main
		properties->append("type") = CTRL_GROUP;
